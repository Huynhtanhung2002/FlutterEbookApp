...
class _GenreScreenState extends ConsumerState<GenreScreen> {
  ...
  void initState() {
    super.initState();
    _fetch(); // Tải dữ liệu khi màn hình được khởi tạo
    _scrollController.addListener(paginationListener); // Lắng nghe sự kiện cuộn trang
  }

  // Hàm để tải dữ liệu từ provider
  void _fetch() {
  ...
  void didUpdateWidget(covariant GenreScreen oldWidget) {
    ...
    if (oldWidget.url != widget.url) {
      _fetch(); // Tải dữ liệu mới khi URL thay đổi
    }
  }
      ...
      ref.watch(genreFeedNotifierProvider(widget.url)).maybeWhen(
        data: (data) {
          ...
          if (!loadingMore) {
            page += 1;
            ref
                .read(genreFeedNotifierProvider(widget.url).notifier) // Gửi yêu cầu phân trang để tải thêm dữ liệu
                .paginate(page);
  ...
  @override
  void dispose() {
    _scrollController.removeListener(paginationListener); // Loại bỏ lắng nghe sự kiện cuộn trang khi widget bị hủy
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    // Kiểm tra xem có thể quay trở lại màn hình trước đó không
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        ...
                        return Padding(
                          ...
                          child: BookListItem(entry: entry), // Hiển thị thông tin về sách
                        );
                      },
                    ),
                    const SizedBox(height: 10.0),
                    if (loadingMore)
                    // Hiển thị widget loading nếu đang tải thêm dữ liệu
